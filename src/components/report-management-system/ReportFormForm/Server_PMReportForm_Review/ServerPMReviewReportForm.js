...
const ServerPMReviewReportForm = ({
  ...
  // Handler to clear all signatures
  const handleClearAllSignatures = () => {
    ...
    if (attendedByCanvasRef.current) {
      const ctx = attendedByCanvasRef.current.getContext('2d');
      ctx.clearRect(0, 0, attendedByCanvasRef.current.width, attendedByCanvasRef.current.height);
    }
    if (approvedByCanvasRef.current) {
      const ctx = approvedByCanvasRef.current.getContext('2d');
      ctx.clearRect(0, 0, approvedByCanvasRef.current.width, approvedByCanvasRef.current.height);
    }
