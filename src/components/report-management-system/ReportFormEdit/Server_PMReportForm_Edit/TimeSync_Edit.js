...
const TimeSync_Edit = ({ data, onDataChange, onStatusChange, stationNameWarehouseID }) => {
  ...
  useEffect(() => {
    if (timeSyncData.length > 0) {
      const existingMachineNames = timeSyncData
  ...
  }, [timeSyncData]); // Only merge existing names when data changes
                      ...
                      return (
                        <FormControl fullWidth size="small">
                          ...
                          <Select
                            ...
                            value={selectValue}
                            onChange={(e) => handleTimeSyncChange(index, 'machineName', String(e.target.value || '').trim())}
                            label="Machine Name"
