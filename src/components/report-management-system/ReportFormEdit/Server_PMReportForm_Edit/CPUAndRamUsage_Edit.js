...
const CPUAndRamUsage_Edit = ({ data, onDataChange, onStatusChange, stationNameWarehouseID }) => {
  ...
  useEffect(() => {
    const allMachineNames = [
      ...memoryUsageData.filter(row => row.machineName && row.machineName.trim() !== '' && !row.isDeleted).map(row => row.machineName.trim()),
      ...cpuUsageData.filter(row => row.machineName && row.machineName.trim() !== '' && !row.isDeleted).map(row => row.machineName.trim())
    ];
    ...
    if (allMachineNames.length > 0) {
      ...
      setServerHostNameOptions(prevOptions => {
        ...
        allMachineNames.forEach(machineName => {
  ...
  }, [memoryUsageData, cpuUsageData]); // Only merge existing names when data changes
                        ...
                        return (
                          <FormControl fullWidth size="small">
                            ...
                            <Select
                              ...
                              value={selectValue}
                              onChange={(e) => handleMemoryUsageChange(index, 'machineName', String(e.target.value || '').trim())}
                              label="Machine Name"
                        ...
                        return (
                          <FormControl fullWidth size="small">
                            ...
                            <Select
                              ...
                              value={selectValue}
                              onChange={(e) => handleCpuUsageChange(index, 'machineName', String(e.target.value || '').trim())}
                              label="Machine Name"
                              disabled={row.isDeleted || loadingServerHostNames}
                              sx={{
                                '& .MuiSelect-select': {
