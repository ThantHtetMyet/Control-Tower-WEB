...
const ASAFirewall_Edit_Review = ({ data, disabled = true, formData }) => {
  ...
  useEffect(() => {
    if (!isInitialized.current) {
      ...
      if (dataToProcess && dataToProcess.length > 0) {
        // Map data to match ASAFirewall_Review structure
        const mappedData = dataToProcess.map(item => ({
          ...
          expectedResultId: item.asaFirewallStatusID || item.ASAFirewallStatusID || item.expectedResultId,
          doneId: item.resultStatusID || item.ResultStatusID || item.doneId,
        ...
        // Initialize with default data for display (matching ASAFirewall_Review)
        setAsaFirewallData([
          { 
            serialNumber: 1,
...
export default ASAFirewall_Edit_Review;